...
describe('JSONDatabase', () => {
  ...
  it('returns the zero address for nonexistent names', () => {
    expect(db.addr('mydao.test', ETH_COIN_TYPE)).toStrictEqual({
      addr: ZERO_ADDRESS,
      ttl: 300,
    });
  });
  ...
  it('resolves exact names', () => {
    expect(db.addr('mydao.eth', ETH_COIN_TYPE)).toStrictEqual({
      addr: TEST_DB['mydao.eth'].addresses[ETH_COIN_TYPE],
      ttl: 300,
    });
  });
